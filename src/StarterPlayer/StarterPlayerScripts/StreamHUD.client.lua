...
local old=playerGui:FindFirstChild("ViewersVsMeHUD");if old then old:Destroy() end
local gui=Instance.new("ScreenGui");gui.Name="ViewersVsMeHUD";gui.ResetOnSpawn=false;gui.IgnoreGuiInset=false;gui.DisplayOrder=999;gui.Parent=playerGui
...
local function text(parent,txt,pos,size,font,ts,color,align)local l=Instance.new("TextLabel");l.BackgroundTransparency=1;l.Position=pos;l.Size=size;l.Font=font or Enum.Font.GothamBold;l.TextSize=ts or 14;l.TextColor3=color or Color3.new(1,1,1);l.Text=txt;l.TextXAlignment=align or Enum.TextXAlignment.Left;l.Parent=parent;return l end
local function panel(name,pos,size,trans)local f=Instance.new("Frame");f.Name=name;f.Position=pos;f.Size=size;f.BackgroundColor3=Color3.fromRGB(8,10,15);f.BackgroundTransparency=trans or .16;f.BorderSizePixel=0;f.Parent=gui;round(f,14);stroke(f,.82);return f end
...
local hpBack=Instance.new("Frame");hpBack.Size=UDim2.fromOffset(300,19);hpBack.BackgroundColor3=Color3.fromRGB(70,20,24);hpBack.BorderSizePixel=0;hpBack.Parent=vitals;round(hpBack,6)
